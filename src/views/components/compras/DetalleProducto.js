...
class DetalleProducto extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.farmaciaSelected && this.state.productoSelected) {
    ...
    if (productoSelected) {
      try {
        ...
        if (result.data) {
          ...
          var productos_propios =
            await this.state.farmaciaSelected.productos.filter(
  ...
  render() {
    ...
    return (
      <>
        {this.state.loading ? (
          <h2 className="m-3 p-2" align="center">
            Loading...
          </h2>
        ) : farmaciaSelected === null || productoSelected === null ? (
          <>
                ...
                {!FarmaciaVendeElProducto(
                  ...
                  <div className="row ">
                    <div className="col-md-4 col-4" align="right">
                      <img
                        src={image_path_server + productoSelected[0].imagen}
                        alt=""
                        style={{ height: '25vw', maxWidth: '100%' }}
                      />
                    </div>
                    <div className="col-md-4 col-8 detalle-producto">
                      <p id="nombre">{productoSelected[0].nombre}</p>
                      <p id="precio">${productoSelected[0].precio}</p>
                      <p id="descripcion">Descripción</p>
                      <small>
                        {this.parseDescripcion(productoSelected[0].descripcion)}
                      </small>
                ...
                <div className="row">
                  <div className="col">
                    <TabsProductosRelacionados
                      ...
                      textbuscador={productoSelected[0].nombre}
                    />
                  </div>
                </div>
              </div>
